...
func (a *OrderedWrapper[T]) Compare(b *OrderedWrapper[T]) generics.Ordering {
	if a.T < b.T {
		return generics.OrderingLessThan
	} else if a.T == b.T {
		return generics.OrderingEqual
	} else {
		return generics.OrderingGreaterThan
	}
}
...
func Comp2[A Ordered](a A, b A) generics.Ordering {
	if a < b {
		return generics.OrderingLessThan
	} else if a == b {
		return generics.OrderingEqual
...
func Max2[S ~[]E, E generics.Ord[E]](vs S) E {
	if len(vs) == 0 {
		panic("no elements")
	}
	...
	var r = vs[0]
	for i := range vs[1:] {
		if generics.GreaterThan(vs[i], r) {
			//if vs[i] > r {
			r = vs[i]
		}
	}
...
func Example() {
	...
	wrapped := generics.MapSlice(unwrapped, Wrap[uint64])
	maxI := Max2(wrapped)
